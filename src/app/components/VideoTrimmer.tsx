...
function VideoTrimmer({ setHandleTrim, rStart, setRstart, rEnd, setRend, inputVideoFile, videoMeta}: any) {
  ...
  return (
      <div className="u-center flex flex-col w-full" style={{ margin: "1rem" }}>
        <label className="text-white text-lg font-semibold mb-2">Select max 20 sec video for analysis</label>
        <Nouislider
            behaviour="tap-drag"
            step={1}
            margin={5}
        ...
        <div className="flex justify-between items-center mt-2">
          <span className="text-white text-sm font-semibold font-mono">{formatTime(rStart)}</span>
          <span className="text-white text-sm font-semibold font-mono">{formatTime(rEnd)}</span>
        </div>
      </div>
  );
}
